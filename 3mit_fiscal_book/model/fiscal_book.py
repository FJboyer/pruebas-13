...
class FiscalBook(models.Model):
   ...
   # company_id = fields.Many2one('res.company', 'Compañia', help='Company', )
    ...
    def update_book_issue_invoices(self, fb_id):
        ...
        for invoice in issue_inv_ids:
            ...
            amount_total = invoice.amount_total * float(tasa)
        ...
        if inv_iva:
                ...
                for iwdl_otro1 in inv_iva:
                  #  busq = self.env['account.move'].search([('id','=',)])
                    if iwdl_otro1 and iwdl_otro1.type == 'in_invoice' or iwdl_otro1.type == 'in_refund':
                      otro = iwdl_otro1.id
                ...
                for iwdl_brw in iwdl_ids:
                    # if iwdl_brw.type == 'in_invoice':
                        ...
                        values = {'iwdl_id': iwdl_brw.id,
                                  ...
                                  'invoice_number': iwdl_brw.invoice_id.supplier_invoice_number if  iwdl_brw.invoice_id.supplier_invoice_number else iwdl_brw.invoice_id.name, # se agrega el campo numero de factura para las facturas fuera de periodo
            ...
            if fbl.iwdl_id.invoice_id:
                ...
                for line in fbl.invoice_id.invoice_line_ids:
                    for tax in line.tax_ids:
                        busq = tax.appl_type
                        if busq:
                            base_sum[fbl.type][busq] += line.price_subtotal * sign * float(tasa)
                            tax_sum[fbl.type][busq] += (line.price_total - line.price_subtotal) * sign * float(tasa)
                        else:
                            raise exceptions.except_orm("Advertencia!", "En los impuestos no se encuentra el Tipo de Alicuota por favor colocar para proceder")
 ...
 #       ait_obj = self.env['account.invoice.tax']
    ...
    def obtener_tasa(self, invoice):
        fecha = invoice.date
        tasa_id = self.env['res.currency'].search([('name', '=', 'Bs.')])
        #tasa = self.env['res.currency.rate'].search([('id', '=', tasa_id.id), ('name', '<=', fecha)], order='id desc', limit=1)
        rate = self.env['res.currency.rate'].search([
            ('currency_id', '=', tasa_id.id),
            ('name', '<=', fecha),
        ], limit=1)
        if not rate:
