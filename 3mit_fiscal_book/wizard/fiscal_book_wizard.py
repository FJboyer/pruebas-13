...
class PurchaseBook(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for h in purchasebook_ids:
            ...
            expediente = ''
            total = 0
            partner = self.env['res.partner'].search([('vat','=', h.partner_vat)])
            ...
            if (partner_1.company_type == 'company' or  partner_1.company_type == 'person') and (partner_1.people_type_company or partner_1.people_type_individual) and  (partner_1.people_type_company == 'pjdo' or partner_1.people_type_individual == 'pnre' or  partner_1.people_type_individual == 'pnnr'):
                ...
                if h.invoice_id:
                    ...
                    if h.doc_type == 'N/CR':
                        total = (h.invoice_id.amount_total)*-1 * float(tasa)
                    else:
                        total = (h.invoice_id.amount_total) * float(tasa)
                ...
                if h.iwdl_id.invoice_id:
                    ...
                    if h.doc_type == 'N/CR':
                        total = (h.iwdl_id.invoice_id.amount_total)*-1 * float(tasa)
                    else:
                        total = (h.iwdl_id.invoice_id.amount_total) * float(tasa)
                    sum_vat_reduced_base += h.vat_reduced_base  # Base Imponible de alicuota Reducida
